package com.zwt.framework.rocketmq.producer;
...
import com.zwt.framework.rocketmq.util.MessageUtil;
...
public class RocketMQMessageProducer {
    ...
    /**
     *
     * @param groupName (producerGroup这个概念发送普通的消息时，作用不大，但是发送分布式事务消息时，比较关键)
     * @param namesrvAddr
     */
    ...
    private SendResult sendMessageSync(Message message, String businessId) throws Exception{
        ...
        try{
            ...
            if(businessId==null){
                ...
                logger.debug("sendMessageSync:"+ MessageUtil.getMessageJsonStr(message));
            }else{
                sendResult = this.producer.send(message, new SelectMessageQueueByHash(), businessId);
                logger.debug("sendMessageSync orderly:"+ MessageUtil.getMessageJsonStr(message));
            }
            return sendResult;
        }catch(Exception e){
            logger.error("",e);
    ...
    private void sendMessageAsync(Message message, String businessId, SendCallback sendCallback) throws Exception{
        ...
        try{
            if(businessId==null){
                ...
                logger.debug("sendMessageAsync:"+ MessageUtil.getMessageJsonStr(message));
            }else{
                this.producer.send(message,  new SelectMessageQueueByHash(), businessId, sendCallback);
                logger.debug("sendMessageAsync orderly:"+ MessageUtil.getMessageJsonStr(message));
            }
        }catch(Exception e){
            logger.error("",e);
            throw e;
