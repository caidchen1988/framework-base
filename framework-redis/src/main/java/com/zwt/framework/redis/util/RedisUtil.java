...
public interface RedisUtil {
     ...
     * @return 返回递增后的值
     */
    double incrFloatby(String key, double value);
     ...
     */
    String setString(String key, String value, int expire);
     ...
     */
    Long setStringIfNotExists(String key, String value);
    ...
    /**
     * 批量的
     * 注意：由于需要对每个key进行计算slots，所以cluster暂时还无法使用pipeline方式
     * @param keys key数组
     * @param values values数组
     * @return 操作状态的集合
     */
    List<String> batchSetString(String[] keys, String[] values);
     ...
     * @return value的集合
     */
    List<String> batchGetString(String[] keys);
     ...
     * @param field 域
     * @param value string value
     * @return 如果 field 是哈希表中的一个新建域，并且值设置成功，返回 1 。如果哈希表中域 field 已经存在且旧值已被新值覆盖，返回0
     */
     ...
     */
    String hashMultipleSet(String key, Map<String, String> hash);
     ...
     */
    List<String> hashMultipleGet(String key, String... fields);
     ...
     */
    Long listPushTail(String key, String... values);
     ...
     */
    Long listPushHead(String key, String... values);
     ...
     * @return 执行结果
     */
    Map<String, List<String>> batchGetAllList(String... keys);
     ...
     * @return 如果命令执行成功，返回 OK 。当 key 不是哈希表(hash)类型时，返回一个错误。
     */
    String hashBinarySet(byte[] key, Map<byte[], byte[]> hash);
     ...
     * @return 一个包含多个给定域的关联值的表，表值的排列顺序和给定域参数的请求顺序一样。
     */
    List<byte[]> hashBinaryGet(byte[] key, byte[]... fields);
     ...
     * @param members
     */
    long scard(String key, String... members);
     ...
     * @return 在设置操作成功完成时，才返回 OK 。
     */
    String setByte(final String key, final byte[] value) ;
     ...
     * @return 在设置操作成功完成时，才返回 OK 。
     */
    String setByteEx(final String key,int seconds, final byte[] value) ;
     ...
     * @return
     */
    Map<byte[], byte[]> hashByteGetAll(final String key) ;
     ...
     * @return
     */
    Long hashByteDel(final byte[] key, final byte[]... fields);
     ...
     * @return 删除成功的条数
     */
    Long delByteKey(final byte[] key);
     ...
     * @return
     */
    Long zaddHashByte(final byte[] key,double score,byte[] member);
     ...
     * @return
     */
    Long zremrangeByScoreHashByte(final byte[] key, double start, double end);
     ...
     * @return
     */
    Long zremHashByte(final byte[] key,final byte[]... member);
     ...
     * @return
     */
    Set<byte[]> zrangeByScoreHashByte(final byte[] key,double min,double max);
    ...
    /**
     * 根据key获取value ,若为null则赋值value,返回null
     * @param key
     * @param value
     * @return
     */
